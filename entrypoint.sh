...
if [ -n "${GIT_URL}" ]; then
  ...
  if [ ! -f "${CONTAINER_ROOT}/package.json" ] && [ ! -d "${CONTAINER_ROOT}/.git" ]; then
    ...
    if [ -z "${GIT_BRANCH}" ]; then
      git clone "${GIT_URL}" "${CONTAINER_ROOT}"
    else
      git clone --single-branch --branch "${GIT_BRANCH}" "${GIT_URL}" "${CONTAINER_ROOT}"
    fi
...
if [ "${SHOULD_INSTALL}" = "1" ] && [ -f "${WORK_DIR}/package.json" ]; then
...
if [ "${NODE_RUN_ENV}" = "production" ]; then
  exec npx next start -p "${SERVER_PORT:-3000}"
else
  exec npx next dev -p "${SERVER_PORT:-3000}"
fi